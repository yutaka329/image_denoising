...
void RandomizedRedundantDCT::init(Size size_, int color_, Size patch_size_)
{
...
RandomizedRedundantDCT::RandomizedRedundantDCT()
{
	rng((uint)cv::getCPUTickCount());
}

RandomizedRedundantDCT::RandomizedRedundantDCT(cv::Size size, int color, cv::Size patch_size_)
{
	rng((uint)cv::getCPUTickCount());
	generateSamplingMaps(size, patch_size_, 20, 0, SAMPLING::FULL);
}

float RandomizedRedundantDCT::getThreshold(float sigmaNoise)
{
	return 2.7f * sigmaNoise;
}

void RandomizedRedundantDCT::operator()(Mat& src_, Mat& dest, float sigma, Size psize, BASIS transform_basis)
{
	Mat src;
...
void RandomizedRedundantDCT::setSamplingMap(Mat& samplingMap, SAMPLING samplingType, int d)
{
...
void RandomizedRedundantDCT::generateSamplingMaps(Size size, Size psize, int number_of_LUT, int d, SAMPLING sampleType)
{
	init(size, 3, psize);
...
void RandomizedRedundantDCT::getSamplingFromLUT(Mat& samplingMap)
{
...
void RandomizedRedundantDCT::colorredundunt(Mat& src_, Mat& dest, float sigma, Size psize, BASIS transform_basis)
{
	Mat src;
	if (src_.depth() != CV_32F)src_.convertTo(src, CV_MAKETYPE(CV_32F, src_.channels()));
	else src = src_;
...
void RandomizedRedundantDCT::body(float *src, float* dest, float Th)
{
	int numThreads = getNumThreads();
...
void RandomizedRedundantDCT::div(float* inplace0, float* inplace1, float* inplace2, float* count, const int size1)
{
	float* s0 = inplace0;
	float* s1 = inplace1;
	float* s2 = inplace2;
...
void RandomizedRedundantDCT::div(float* inplace0, float* inplace1, float* inplace2, float* inplace3, float* count, const int size1)
{
	float* s0 = inplace0;
	float* s1 = inplace1;
	float* s2 = inplace2;
